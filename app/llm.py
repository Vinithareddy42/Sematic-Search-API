...
def build_llm_params(model: str, messages):
    family = os.getenv("LLM_FAMILY", "openai_chat").lower()
    # default temperature from env, fallback 0.3
    try:
    ...
    if family == "openai_chat":
        # GPT‑5 models: temperature must be default (1) only
        if model.startswith("gpt-5"):
            # do NOT send temperature at all
            logging.info("GPT‑5 model detected; omitting temperature for compatibility")
        else:
            params["temperature"] = temperature

    # Anthropic, local, etc. can be customized here later
    return params
...
def generate_summary(products: List[Dict[str, Any]], user_query: str) -> str:
    ...
    if api_key:
        try:
            ...
            logging.info("LLM: calling OpenAI model=%s", model)  # <--- add this
            ...
            params = build_llm_params(model, messages)
